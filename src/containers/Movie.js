...
//Функция для отображения конкретного фильма
const Movie = ({
  location,
  geral,
  match,
  ...
  // Получени id фильма и рекомендованных фильмов при изменении идентификатора в URL
  useEffect(() => {
    scroll.scrollToTop({
      smooth: true,
      delay: 500,
  ...
  //Получение рекомендованных фильмов каждый раз при смене url
  useEffect(() => {
    getRecommendations(match.params.id, params.page);
  ...
  return (
    <Wrapper>
        ...
        <MovieWrapper>
          ...
          <ImageWrapper style={!loaded ? { display: 'none' } : {}}>
            <MovieImg
              ...
              // Если отсутсвует изображение,то  устанавливаю значение true
            />
          </ImageWrapper>
          <MovieDetails>
            <HeaderWrapper>
            ...
            <Playground percentage={movie.vote_average * 10}/>
            <DetailsWrapper>
      ...
      <Header title="Рекомендованные фильмы:" subtitle="movies" />
  ...
  if (history.action === 'PUSH') {
    return (
      <div onClick={history.goBack}>
        <Button title="Назад" solid left icon="arrow-left" />
      </div>
    );
  }
}
...
// Функция для того чтобы отобразить только год
